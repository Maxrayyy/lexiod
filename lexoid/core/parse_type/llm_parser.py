...
def create_response(
    ...
    clients = {
        ...
        "deepseek": lambda: OpenAI(
            base_url=os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com"),
            api_key=os.environ["DEEPSEEK_API_KEY"],
        ),
        "minimax": lambda: OpenAI(
...
def parse_with_api(path: str, api: str, **kwargs) -> Dict:
    ...
    Args:
        ...
        api (str): API provider name, including OpenAI-compatible providers
            such as "deepseek" and "minimax"
    ...
    for page_num, image_url in images:
        if api in {"openai", "deepseek", "minimax", "ollama"}:
