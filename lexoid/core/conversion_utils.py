...
_DOC_ORIENTATION_CLASSIFIER = None


def _get_doc_orientation_classifier():
    global _DOC_ORIENTATION_CLASSIFIER
...
def normalize_document_orientation(
    image: Image.Image,
    detector: Optional[OrientationDetector] = None,
    min_confidence: float = 0.80,
) -> Image.Image:
    """Rotate a classified page upright, leaving uncertain pages unchanged."""
    orientation, confidence = (detector or detect_document_orientation)(image)
    if orientation not in (0, 90, 180, 270):
        raise ValueError(f"Unsupported document orientation: {orientation}")
    if confidence < min_confidence or orientation == 0:
        return image
    logger.info(
        f"Auto-orienting document page by {orientation} degrees "
...
def convert_pdf_page_to_base64(
    ...
    if auto_orient:
        pil_image = normalize_document_orientation(
            pil_image, detector=orientation_detector
        )
    ...
    if is_pdf:
        ...
        try:
            images = [
                (
                    page_num,
                    f"data:image/png;base64,{convert_pdf_page_to_base64(pdf_document, page_num, max_dimension, auto_orient=auto_orient)}",
                )
