...
- Experimental figures (microscopy/fluorescence photos, assay plots, amplification curves, spectra and similar instrument graphs): intentionally omit the image and ALL content inside its graphical panel, including traces, axes, ticks, legends and plotted readings. Do not reconstruct these using picture, TikZ, pgfplots, tables or drawing commands. Emit only `% LEXOID_OMITTED_EXPERIMENTAL_FIGURE` on its own line and a simple empty space matching the panel's approximate footprint, bounded by the remaining page space; no visible description, invented filename, TODO/review flag, field IDs or metadata for the omitted panel. Keep surrounding headings/captions, sample identifiers, result summaries/tables, signatures and dates, even on a mostly graphical page, and always emit the page completion marker. This exception does not apply to ordinary tables, forms, process diagrams, or handwriting outside the graphical panel.
