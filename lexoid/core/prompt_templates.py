...
- Experimental figures (microscopy/fluorescence photos, assay plots, amplification curves, spectra and similar instrument graphs): intentionally omit ALL content inside the graphical panel, including traces, axes, ticks, legends and plotted readings. Represent each panel with an EMPTY RECTANGULAR FRAME using `\LexoidExperimentalFigure{<width>}{<height>}`; this macro is supplied by the pipeline, do not define it. Match the source panel's approximate footprint and arrangement, with width at most the current `\linewidth` and height bounded by the remaining page space. Put `% LEXOID_OMITTED_EXPERIMENTAL_FIGURE` on its own line before the frame. Do not redraw the image using picture, TikZ, pgfplots or drawing commands, and do not emit a visible description, invented filename, TODO/review flag, field IDs or metadata for the omitted interior. Keep surrounding headings/captions, sample identifiers, result summaries/tables, signatures and dates, even on a mostly graphical page, and always emit the page completion marker. This exception does not apply to ordinary tables, forms, process diagrams, or handwriting outside the graphical panel.
- Other figures: if a filename is available, use \includegraphics[width=\linewidth]{<filename>}; otherwise add a % TODO placeholder.
...
- Assay and instrument tables: preserve the printed grid and every visible cell, including empty cells. Do not treat literal backslashes in printed values as TeX commands; transcribe them as visible text with safe escaping. Every tabular row must end with exactly one TeX row break, and row breaks must stay outside `\fieldvalue{...}` and `\handwritten{...}`. Never use a row break to split a value across cells. Never invent a value to repair a table; keep the original text and add `% TODO` when a cell is unreadable.
