...
def test_parse_stdout_stderr_separation():
    ...
    parsing_line = next(
        line for line in result.stderr.splitlines() if "🔄 Parsing" in line
    )
    assert re.match(
...
def test_provider_api_key_env_var(provider, expected_env_var):
    from lexoid.cli import api_key_env_var

    assert api_key_env_var(provider) == expected_env_var
...
def test_core_model_provider_inference(model, expected_provider):
    from lexoid.core.utils import get_api_provider_for_model

    assert get_api_provider_for_model(model) == expected_provider
