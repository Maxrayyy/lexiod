...
For MiniMax accounts in mainland China, set ``MINIMAX_BASE_URL`` to
``https://api.minimaxi.com/v1``. DeepSeek V4 and MiniMax M2 are currently
text-only, so direct PDF/image parsing requires a vision-capable provider.
